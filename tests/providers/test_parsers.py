...
def test_heuristic_tool_parser_raw_json_simple():
    """Test parsing a simple JSON tool call from text."""
    parser = HeuristicToolParser()
    text = 'Let\'s write to a file: {"name": "write_file", "arguments": {"AbsolutePath": "/foo/bar.txt", "CodeContent": "hello"}} and then we are done.'
    filtered, tools = parser.feed(text)
    tools.extend(parser.flush())

    assert len(tools) == 1
    assert tools[0]["name"] == "write_file"
    assert tools[0]["input"] == {"AbsolutePath": "/foo/bar.txt", "CodeContent": "hello"}
    assert "Let's write to a file:" in filtered
    assert "and then we are done." in filtered


def test_heuristic_tool_parser_raw_json_mixed_with_web():
    """Test raw JSON tool parser mixed with WebSearch format."""
    parser = HeuristicToolParser()
    text = 'First WebSearch {"query": "test"}. Next {"name": "read_file", "arguments": {"AbsolutePath": "/tmp/a.txt"}}'
    _filtered, tools = parser.feed(text)
    tools.extend(parser.flush())
