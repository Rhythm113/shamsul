...
def serve() -> None:
    """Start the FastAPI server (registered as `shamsul-server` script)."""
...
def init() -> None:
    """Scaffold config at ~/.shamsul/.env (registered as `shamsul-init`)."""
    ...
    if migrated_from is not None:
        ...
        print("Edit it to set your model preferences, then run: shamsul-server")
        return
